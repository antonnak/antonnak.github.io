var pos = 0;
    ...
    function checkCollisions(item) {
        if (item.position.x + item.velocity.x + item.newimg.width > window.innerWidth) {item.velocity.x = -item.velocity.x; item.direction = 1};
        if (item.position.x + item.velocity.x < 0) {item.velocity.x = -item.velocity.x; item.direction = 0}; 
